...
export function Time({ seconds, frozen = false }: { seconds: number | undefined, frozen?: boolean }) {
    ...
    const h = Math.floor(secondsState / 3600).toString().padStart(2, '0');
    const m = Math.floor((secondsState % 3600) / 60).toString().padStart(2, '0');
    const s = Math.floor(secondsState % 60).toString().padStart(2, '0');
